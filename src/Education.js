import React from "react";
import "./Education.css";

function Education() {
  return (
    <div className="service component__space" id="Education">

      <div className="heading">
        <h1 className="heading">Educational Details</h1>
        <p className="heading p__color">
         
        </p>
        <p className="heading p__color">
         
        </p>
      </div>

      <div className="container">
        <div className="row">
          <div className="col__3">
            <div className="service__box pointer">
              <div className="icon">
                <svg
                  stroke="currentColor"
                  fill="none"
                  strokewidth="2"
                  viewBox="0 0 24 24"
                  strokelinecap="round"
                  strokelinejoin="round"
                  height="1em"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                 <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                  <circle cx="9" cy="7" r="4"></circle>
                  <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
                  <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                </svg>
              </div>
              <div className="service__meta">
                <h1 className="service__text">Degree</h1>
                <p className="p service__text p__color">
                  BE (CSE)
                </p>
                <p className="p service__text p__color">
                  Year Of Passing: 2017-2021
                </p>
                <p className="p service__text p__color">CGPA: 7.53%.</p>
                <p className="p service__text p__color">SBM College Of Engg & Tech</p>
                <p className="p service__text p__color">Dingigul.</p>
              </div>
            </div>
          </div>

          <div className="col__3">
            <div className="service__box pointer">
              <div className="icon">
                <svg
                  stroke="currentColor"
                  fill="none"
                  strokewidth="2"
                  viewBox="0 0 24 24"
                  strokelinecap="round"
                  strokelinejoin="round"
                  height="1em"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                 <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                  <circle cx="9" cy="7" r="4"></circle>
                  <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
                  <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                </svg>
              </div>
              <div className="service__meta">
                <h1 className="service__text">Higher Secondary</h1>
                <p className="p service__text p__color">
                  Year Of Passing: 2017
                </p>
                <p className="p service__text p__color">
                  Percentage: 69.58%
                </p>
                <p className="p service__text p__color">ST.John Paul Higher Secondary School</p>
                <p className="p service__text p__color">Dindigul.</p>
              </div>
            </div>
          </div>

          <div className="col__3">
            <div className="service__box pointer">
              <div className="icon">
                <svg
                  stroke="currentColor"
                  fill="none"
                  strokewidth="2"
                  viewBox="0 0 24 24"
                  strokelinecap="round"
                  strokelinejoin="round"
                  height="1em"
                  width="1em"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                  <circle cx="9" cy="7" r="4"></circle>
                  <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
                  <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                </svg>
              </div>
              <div className="service__meta">
                <h1 className="service__text">SSLC</h1>
                <p className="p service__text p__color">
                  Year Of Passing: 2015
                </p>
                <p className="p service__text p__color">
                 Percentage: 80.6%
                </p>
                <p className="p service__text p__color">St.John Paul higher Secondary School</p>
                <p className="p service__text p__color">Dindigul.</p>
              </div>
            </div>
          </div>

          </div>
        </div>
      
   
    </div>  
  );
}

export default Education;
